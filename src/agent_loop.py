...
# Core tools that are ALWAYS available in agent mode regardless of RAG results.
# Prevents the agent from losing basic capabilities (bash, web, files, documents)
# when RAG fails, times out, or keyword fallback selects the wrong tools.
# Fixes #3041: agent hallucinates instead of executing tools.
_AGENT_MODE_CORE_TOOLS = frozenset({
    "bash", "python", "web_search", "web_fetch",
    "read_file", "write_file", "edit_file", "grep", "glob", "ls",
    "create_document", "edit_document", "update_document",
    "ask_user", "update_plan", "manage_memory",
})
...
def _recent_context_for_retrieval(messages: List[Dict], max_user: int = 7, max_chars: int = 1200) -> str:
...
async def stream_agent_loop(
    ...
    if not guide_only and not _relevant_tools and bool(_intent.get("low_signal")):
        from src.tool_index import ALWAYS_AVAILABLE
        _relevant_tools = set(ALWAYS_AVAILABLE) | _AGENT_MODE_CORE_TOOLS
        if workspace:
            ...
            logger.info("[tool-rag] Low-signal but workspace active; including core + file tools")
        else:
            logger.info("[tool-rag] Low-signal agent message; using always-available + core tools")
                ...
                if _retrieval_query:
                    try:
                        _relevant_tools = await asyncio.wait_for(
                            asyncio.to_thread(tool_idx.get_tools_for_query, _retrieval_query, 14),
                            timeout=_TOOL_SELECTION_TIMEOUT_SECONDS,
                        )
                        logger.info(f"[tool-rag] Retrieved tools for query: {sorted(_relevant_tools - ALWAYS_AVAILABLE)}")
                    except asyncio.TimeoutError:
                        logger.warning(
                            "[tool-rag] Retrieval exceeded %.1fs; falling back to core tools",
                            _TOOL_SELECTION_TIMEOUT_SECONDS,
                        )
                        _relevant_tools = set(_AGENT_MODE_CORE_TOOLS)
        except Exception as e:
    ...
    # Always include core tools to prevent agent from losing basic capabilities (#3041).
    if not guide_only and not _relevant_tools and _retrieval_query:
        ...
        logger.info(f"[tool-rag] Keyword fallback selected: {sorted(_relevant_tools - _AGENT_MODE_CORE_TOOLS)}")
